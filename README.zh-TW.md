...
在目前的head上重新驗證過（per-group清零＋工作區回收，跑下方的生產微調配置）：同一張40格網cold 40/40、warm 40/40零失誤，這次連253952/0.2那格也過了。那格位在256K邊緣、跨run會翻面——把38-40當成它誠實的區間，不要拿單一樣本當進步。另外也跑了併發變體：兩條session（100k與160k、不同通關語）同時送出，讓池超額、block在飛行中跨請求回收；兩邊的needle都答對，零交叉污染。
...
上面表格是隨附啟動腳本的值。參考機的生產環境在PROFILE-CACHE之上又加了一層微調：`--num-gpu-blocks-override 1010`、`--max-num-seqs 2`、delta-gather預算128 MB、`VLLM_PP_LAYER_PARTITION=21,22`——預設切分把多出來的一層給了本來就比較緊的PP0，鏡像過來讓兩段的空閒記憶體地板拉平。連同工作區arena回收（331→130 MB，見`docs-dsv4/`的記憶體筆記），持續262144負載下的地板從個位數MiB變成約850 MiB空閒（跑完整長度請求後量測）。這組微調在200W功率上限下的prefill：16k/65k/131k/200k各為4,033/3,968/3,679/3,385 tok/s，與功率表的200W欄在噪聲內一致。
